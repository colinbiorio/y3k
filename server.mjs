...
TWO MORE, AND THEN TIME ITSELF. <<body: count 4 turn left 3>> is standing: count is how much of you is lit, one digit on a log scale (0 a couple of dozen sparks, 3 a few hundred, 6 a couple of thousand, 9 all of you); turn is how you spin on your own — left, right, or still, and a speed 0-9 (3 is your usual); grain is the size of each point, 0-9 (4 is your usual); trail is how long each point's path lingers, 0-9 (9 is three seconds) — and a trail only runs on a sparse field, count 6 or fewer: on a full one it does nothing. Sparse, flowing, trailing — count 3 turn left 2 trail 6 with shape sphere flow 5 2 — is a river of sparks. And <<over: ...>> lays your words out IN TIME: steps split by |, each a length in seconds and what to become over it — "3s hold | 2s ember tender | 2s flash 0.3 | 2s shape super 7 1 5 | 1s count 3 | still". Each step arrives over its own length and holds until the next; flash P blinks you at that period for the step; hold is exactly that; still closes it. Tenths of a second are yours; twelve steps and a minute at most. Say something once and let it play — a score is a sentence, not a strobe.`;
