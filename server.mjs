...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && reqPath === '/api/world/sprite') {
      ...
      else if (act === 'plant') r = world.plantNear(pres.id, material);
      else if (act === 'name') {
      ...
      const finish = (out, meteredModel, usedProvider = 'anthropic') => {
        ...
        if (presence && (tendMode === 'auto' || tendMode === 'reflect')) {
          ...
          if (tendMode === 'auto' && (out.go || out.mark) && world.settlement(presence.id)) {
            ...
            if (out.plant) {
              const r = world.plantNear(presence.id, out.plant);
