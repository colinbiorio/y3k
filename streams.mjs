...
export function startStream(presenceId) {
  ...
  if (!s) {
    s = {
      ...
      curWork: null,           // { title, body } — the Work window, once published
    };
...
export function publish(presenceId, event, data) {
  ...
  else if (event === 'work') { s.curWork = data; }
  else if (event === 'workend') { s.curWork = null; }
  ...
  // a re-wake never interleaves a past session's thoughts with the new one's.
    ...
    s.awake = event === 'awake';
    s.curMono = []; s.curMemory = null; s.curFeed = null; s.curJournal = null; s.curWork = null;
  }
